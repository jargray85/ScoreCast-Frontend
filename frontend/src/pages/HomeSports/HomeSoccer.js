...
function HomeSoccer() {
  ...
  useEffect(() => {
    const fetchSportsData = async () => {
      ...
      fetch(
        "https://flashlive-sports.p.rapidapi.com/v1/events/live-list?locale=en_INT&timezone=-4&sport_id=1",
        options
      )
        ...
        {dataSport.map((sport, index) => (
          <div className="event-card" key={index}>
            ...
            {sport.EVENTS.map((event, index) => {
              //display current stage of the soccer game
              let currentStage = "";
              if (event.STAGE === "FIRST_HALF") {
                currentStage = "First Half";
              } else if (event.STAGE === "SECOND_HALF") {
              ...
              return (
                <LinkHomeStyled to={`/soccer/show/${event.EVENT_ID}`}>
                  <div className="event-details" key={index}>
                    ...
                    <h3 className="status">{event.EVENT_STATUS}</h3>
                  </div>
                </LinkHomeStyled>
              );
            })}
